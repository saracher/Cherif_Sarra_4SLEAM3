pipeline {
    ...
    stages {
        ...
        stage('Deploy to Kubernetes') {
            steps {
                sh "kubectl --kubeconfig=$KUBECONFIG_PATH set image deployment/$DEPLOYMENT_NAME $DEPLOYMENT_NAME=$DOCKERHUB_REPO:$IMAGE_TAG -n $KUBE_NAMESPACE"
                sh "kubectl --kubeconfig=$KUBECONFIG_PATH rollout status deployment/$DEPLOYMENT_NAME -n $KUBE_NAMESPACE"
            }
        }
    }
